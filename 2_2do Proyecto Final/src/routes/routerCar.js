...
const containerFirebase = new CarsDaoFirebase()
const containerFirebaseProducts = new ProductsDaoFirebase()
...
routerCar.get("/1/products", async (req, res) => {
  ...
  //Firebase
  const car = await containerFirebase.getAll()
  car.length === 0 ? (products = false) : (products = car[0].products)
...
routerCar.post("/:id/products", async (req, res) => {
  ...
  //Firebase
  const car = await containerFirebase.getAll()
  const product = await containerFirebaseProducts.getById(idProduct)
  if (car.length === 0) {
    const car = {
      timestamp: Date.now(),
      products: [product],
    }
    containerFirebase.save(car)
  } else {
    let isProductFound = false
    ...
    if (!isProductFound) {
      car[0].products.push(product)
      containerFirebase.update(car[0])
    }
  }
...
routerCar.delete("/1/products/:id_prod", async (req, res) => {
  ...
  if (car[0].products.length === 0) {
    containerFirebase.deleteById(car[0].id)
  } else {
    containerFirebase.update(car[0])
  }
})
